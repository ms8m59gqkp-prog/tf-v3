/**
 * PATCH /api/admin/orders/[id]/inspection — 주문 아이템 검수 업데이트
 * WHY: 검수 결과(상태/가격/사유) 반영
 * HOW: withAdmin → UUID + Zod → order.updateInspection → ok
 * WHERE: admin/orders/detail 검수 탭
 */
import type { NextRequest } from 'next/server'
import { withAdmin } from '@/lib/api/middleware'
import { ok, errFrom, validationErr } from '@/lib/api/response'
import { uuidSchema } from '@/lib/utils/validation'
import { UpdateInspectionSchema } from './schema'
...
export const PATCH = withAdmin<{ id: string }>(async (req: NextRequest, { params }) => {
  try {
    ...
    const result = await orderService.updateInspection(itemId, fields)
    return ok(result)
