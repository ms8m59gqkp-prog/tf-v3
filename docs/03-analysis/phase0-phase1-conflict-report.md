...
| # | 대상 | 수정 내용 | 긴급도 |
|---|------|----------|--------|
| 1 | `settlement.ts` | `SETTLEMENT_STATUSES`에 `'draft'` 추가 | **즉시** |
| 2 | `settlement.ts` | `Settlement`에 `itemCount?: number` 추가 | 높음 |
| 3 | `settlement.ts` | `Settlement`에 `periodStart/periodEnd` 추가 | 높음 |
| 4 | `settlement.repo.ts` | SETTLEMENT_COLUMNS: `settlement_period_start, settlement_period_end, item_count` | **즉시** |
| 5 | `sellers.repo.ts` | COLUMNS에 `updated_at` 추가 + mapRow 수정 | 중간 |
| 6 | `db.test.ts` | settlement mock에 'draft' 테스트 추가 | 높음 |
